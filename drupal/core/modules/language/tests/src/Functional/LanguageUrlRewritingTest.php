...
class LanguageUrlRewritingTest extends BrowserTestBase {
  ...
  public function testDomainNameNegotiationPort() {
    ...
    // In case index.html is part of the URLs, we need to adapt the asserted
    // URLs as well.
    $index_php = strpos(\Drupal::url('<front>', [], ['absolute' => TRUE]), 'index.html') !== FALSE;
    ...
    $expected = ($index_php ? 'http://example.fr:88/index.html' : 'http://example.fr:88') . rtrim(base_path(), '/') . '/';
    ...
    $expected = $index_php ? 'http://example.fr:90/index.html' : 'http://example.fr:90' . rtrim(base_path(), '/') . '/';
